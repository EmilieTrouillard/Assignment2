double jacobi(int N, double **array_in, double **array_out, int max_iter, double threshold, double **fmatrix) {
/* array_in and array_out are (N+2)*(N+2) */
	...
	d = 0.0;
	//Oprhaned parallel, is only entered if specified at runtime
	#pragma omp for collapse(2) private(j) reduction(+:matSum)
    	for(i = 1; i <= N; i++){ 
	    for(j = 1; j <= N; j++){
